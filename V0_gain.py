...
for i in range(len(Fre_list)):
        ...
        ac_line1 = circuit.AcLine('input_RF', 'RF_PORT', circuit.gnd, rms_voltage=dBm_to_voltage(-25) @ u_V,
                                  frequency=Fre_list[i] @ u_GHz)
        ...
        analysis = simulator.transient(step_time=0.0005 @ u_ns, end_time=0.01 @ u_us)

        # 计算功率波形的平均值
        RF_input_power_avg = abs(
            np.mean((np.array(analysis['RF_PORT']) - np.array(analysis['RF_PORT_'])) / 50 * np.array(analysis['RF_PORT_'])) * 1e8)
        IF_output_power_avg = abs(np.mean((np.array(analysis['IF_PORT']) ** 2) / 50) * 1e8)
...
plt.plot(Fre_list, mG)
plt.show()
concetated = np.concatenate((Fre_list.reshape(-1,1), mG.reshape(-1,1)), axis=1)
print(concetated.shape)
np.save('Gain.npy', concetated)
